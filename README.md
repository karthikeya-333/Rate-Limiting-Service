...
PostgreSQL is used to store projects, API keys, and rate-limit configurations, while Redis is used to keep fast, temporary counters for request tracking. Redis automatically resets counts when the time window expires, making the checks efficient.

The service also supports idempotency, so repeated requests with the same idempotency key are not counted multiple times. This is useful when clients retry requests due to network issues.
